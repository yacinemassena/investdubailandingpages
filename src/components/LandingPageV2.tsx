  ...
  return (
    <div className="fixed top-0 left-0 right-0 bg-black z-[60] py-2">
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-center gap-2">
        <span className="text-label text-white/50 mr-4">Pages:</span>
          ...
          <div key={num} className="relative group">
            ...
            <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-[70]">
              <div className="bg-white text-black rounded-lg px-4 py-3 shadow-xl min-w-[200px] text-center">
                ...
                <p className="text-body font-bold text-zinc-500 mb-1">Page {num}</p>
                <p className="text-body font-medium">{pageDescriptions[num].topic}</p>
                <p className="text-body text-zinc-400 mt-1 uppercase tracking-wider">KPI: {pageDescriptions[num].kpi}</p>
              </div>
            </div>
          </div>
        ))}
...
const StatCard = ({ label, value, subValue }: StatProps) => (
  <div className="flex flex-col gap-1 border-l border-zinc-200 pl-6 py-2 first:border-0 first:pl-0">
    <p className="text-body text-zinc-500 uppercase tracking-widest font-semibold">{label}</p>
    <p className="text-stat tracking-tight">{value} <span className="text-body font-medium text-zinc-400">{subValue}</span></p>
  </div>
);
  ...
  <section className="relative pt-32 pb-16 md:pt-48 md:pb-24 overflow-hidden">
    <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-2 gap-12 items-center">
      <div className="relative z-10 flex flex-col gap-8">
        <div className="flex items-center gap-2 bg-zinc-50 border w-fit px-3 py-1 rounded-full text-body font-medium">
        ...
        <p className="text-body-lg text-zinc-600 max-w-xl">
          {content.answerCapsule.substring(0, 200)}...
        </p>
        <div className="flex flex-wrap gap-4 mt-4">
          <a href="https://investdubai.my.smartcrowd.ae/" target="_blank" rel="noopener noreferrer" className="text-white px-8 py-4 rounded-full font-bold text-body tracking-wide hover:opacity-90 flex items-center gap-2 group transition-all" style={{ backgroundColor: '#28de91' }}>
            {language === 'en' ? 'Get Started' : "S'inscrire"}
            <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
          </a>
          <a href="https://calendly.com/acheffi/invest-dubai" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-6 py-4 rounded-full font-bold text-body text-zinc-500 hover:text-black transition-colors">
            <Phone size={14} />
      ...
      <div className="relative">
        ...
        <div className="absolute -bottom-6 -left-6 bg-white p-6 rounded-2xl shadow-xl border border-zinc-100 hidden md:block">
          <p className="text-body font-semibold text-zinc-400 mb-1">{content.metricLabel}</p>
          <p className="text-stat">{content.metricValue} <span className="text-body font-medium text-zinc-400">{content.metricSubtext}</span></p>
        </div>
      </div>
    </div>
  </section>
...
const HowItWorks = ({ content }: { content: LanguageContent }) => (
  <section id="how-it-works" className="py-12 md:py-20 bg-zinc-900 overflow-hidden text-white">
        ...
        {content.howItWorks.steps.map((step, idx) => (
          ...
          >
            <div className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center mb-2 transition-all" style={{ '--hover-bg': '#28de91' } as React.CSSProperties}>
              <span className="text-body font-bold">{String(idx + 1).padStart(2, '0')}</span>
            </div>
            <h3 className="text-h3">{step.title}</h3>
            <p className="text-body-sm text-zinc-400 whitespace-pre-line">{step.description}</p>
          </div>
        ))}
      </div>
    </div>
...
const TrustSection = ({ content, language }: { content: LanguageContent; language: 'en' | 'fr' }) => (
  <section className="py-12 md:py-20 bg-white">
    <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-2 gap-20 items-center">
      <div>
        <p className="text-label text-zinc-400 mb-6">Why Trust Us</p>
        ...
        <div className="flex flex-col">
          {content.trust.items.map((item, idx) => (
            <div key={idx} className="flex items-center gap-4 py-6 border-b border-zinc-100 last:border-0 group cursor-default">
              ...
              <div>
                <p className="text-body-lg font-medium text-zinc-700">{item.title}</p>
                <p className="text-body text-zinc-500 whitespace-pre-line">{item.description}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="bg-white rounded-3xl p-8 md:p-12 shadow-sm border-2 relative overflow-hidden" style={{ borderColor: '#28de91' }}>
        ...
        <div className="relative z-10">
          <div className="mb-8">
            <p className="text-label text-zinc-400 mb-3">{language === 'en' ? 'Ready to Start?' : 'Prêt à commencer ?'}</p>
          ...
          <ul className="flex flex-col gap-3 mb-8">
            {(language === 'en' ? ['2-minute signup', 'Start from 4,000 AED', 'Secure SPV ownership', '100% remote process'] : ['Inscription en 2 minutes', 'Dès 4 000 AED', 'Propriété légale sécurisée', '100% à distance']).map((item, i) => (
              <li key={i} className="flex items-center gap-3 text-body font-medium text-zinc-700">
          ...
          <div className="flex flex-col gap-3">
            <a href="https://investdubai.my.smartcrowd.ae/" target="_blank" rel="noopener noreferrer" className="w-full text-black py-4 rounded-full font-bold text-body tracking-widest hover:opacity-90 transition-colors block text-center" style={{ backgroundColor: '#28de91' }}>
              {language === 'en' ? 'Get Started' : "S'inscrire"}
            </a>
            <a href="https://calendly.com/acheffi/invest-dubai" target="_blank" rel="noopener noreferrer" className="w-full py-3 rounded-full font-medium text-body text-zinc-500 hover:text-zinc-700 transition-colors flex items-center justify-center gap-2">
              <Phone size={14} />
              {language === 'en' ? 'Book a Call with an Advisor' : 'Appeler un conseiller'}
            </a>
          </div>
...
const FAQItem = ({ question, answer }: { question: string; answer: string }) => {
  ...
  return (
    <div className="border-b border-zinc-100">
      ...
      >
        <span className={`text-body-lg font-bold pr-8 transition-colors ${isOpen ? 'text-black' : 'text-zinc-600 group-hover:text-black'}`}>{question}</span>
      ...
      <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isOpen ? 'max-h-[600px] pb-6' : 'max-h-0'}`}>
        <p className="text-zinc-600 leading-relaxed max-w-3xl whitespace-pre-line">{answer}</p>
      </div>
    </div>
  );
};
...
const FAQ = ({ content }: { content: LanguageContent }) => (
  <section id="faq" className="py-12 md:py-20 bg-zinc-50">
    <div className="max-w-4xl mx-auto px-6">
      <div className="text-center mb-16">
        <p className="text-label text-zinc-400 mb-6">FAQ</p>
        <h2 className="text-h2 mb-4">{content.faq.title}</h2>
        <p className="text-body-lg text-zinc-500">{content.faq.subtitle}</p>
      </div>
  ...
  <section className="py-12 md:py-20">
    <div className="max-w-5xl mx-auto px-6 bg-zinc-900 rounded-[48px] p-12 md:p-24 text-center relative overflow-hidden">
      <div className="relative z-10 flex flex-col items-center gap-8">
        <h2 className="text-h2 text-white">{content.nextStep.title}</h2>
        <p className="text-body-lg text-zinc-400 max-w-xl">{content.nextStep.subtitle}</p>
        <div className="flex flex-col sm:flex-row gap-4 mt-4">
          <a href="https://investdubai.my.smartcrowd.ae/" target="_blank" rel="noopener noreferrer" className="text-black px-10 py-5 rounded-full font-bold text-body tracking-widest hover:scale-105 transition-transform" style={{ backgroundColor: '#28de91' }}>
            {language === 'en' ? 'Get Started' : "S'inscrire"}
          </a>
          <a href="https://calendly.com/acheffi/invest-dubai" target="_blank" rel="noopener noreferrer" className="border border-white/20 text-white px-10 py-5 rounded-full font-bold text-body tracking-widest hover:bg-white/10 transition-all flex items-center justify-center gap-2">
            <Phone size={14} />
            {language === 'en' ? 'Book a Call with an Advisor' : 'Appeler un conseiller'}
          </a>
        </div>
        <p className="text-zinc-500 text-body uppercase tracking-widest mt-4">{content.nextStep.footnote}</p>
      </div>
    </div>
  </section>
);
...
export const LandingPageV2: React.FC<LandingPageV2Props> = ({ config }) => {
  ...
  const [language] = useState<'en' | 'fr'>(defaultLang);
