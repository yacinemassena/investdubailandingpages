...
const Navbar = ({ 
  ...
  <nav className={`fixed left-0 right-0 bg-white/90 backdrop-blur-md border-b border-gray-100 z-50 ${isInIframe() ? 'top-0' : 'top-10'}`}>
    <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
      <Logo />
      <div className="hidden md:flex items-center gap-8 text-label">
        ...
        <a href="https://investdubai.my.smartcrowd.ae/" target="_blank" rel="noopener noreferrer" className="bg-black text-white px-6 py-2.5 rounded-full hover:bg-gray-800 transition-all shadow-sm">
  ...
  <section className="pt-48 pb-24 px-6 relative overflow-hidden bg-white">
    ...
    <div className="max-w-7xl mx-auto grid lg:grid-cols-12 gap-16 items-center relative z-10">
      <div className="lg:col-span-7 space-y-10">
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-gray-50 border border-gray-100 text-body font-bold uppercase tracking-[0.2em]">
          <span className="w-1.5 h-1.5 rounded-full bg-[#28de91] animate-pulse"></span>
          {content.badge}
        </div>
        
        <h1 className="text-display-1 text-black">
          {content.headline}
        </h1>
        ...
        <div className="flex flex-col sm:flex-row gap-5 pt-4">
          <a href="https://investdubai.my.smartcrowd.ae/" target="_blank" rel="noopener noreferrer" className="bg-black text-white px-10 py-5 rounded-full font-bold uppercase tracking-widest text-body flex items-center justify-center gap-3 group hover:bg-gray-900 transition-all transform hover:-translate-y-0.5 shadow-xl shadow-black/5">
          ...
          <button 
            ...
            className="bg-white border border-gray-200 text-black px-10 py-5 rounded-full font-bold uppercase tracking-widest text-body flex items-center justify-center gap-3 hover:border-[#28de91] transition-all">
            <ArrowRight size={16} className="text-[#28de91] rotate-90" />
  ...
  <section id="how-it-works" className="py-32 bg-[#f9f9f8] px-6">
    <div className="max-w-7xl mx-auto">
      <div className="max-w-3xl mb-20">
        <h2 className="text-h2 mb-8 text-black">{content.howItWorks.title}</h2>
        <p className="text-body-lg text-gray-600">
          {content.howItWorks.subtitle}
        </p>
        {content.howItWorks.transitionBridge && (
          <div className="mt-8 p-6 bg-white rounded-2xl border-l-4 border-[#28de91] shadow-sm">
            <p className="text-body-lg text-gray-700 font-medium italic">
              {content.howItWorks.transitionBridge}
            </p>
          </div>
        )}
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        {content.howItWorks.steps.map((step, idx) => (
          <div key={idx} className="bg-white p-10 rounded-[2rem] border border-gray-100 flex flex-col gap-6 hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
            <div className="w-12 h-12 rounded-full bg-[#28de91]/10 flex items-center justify-center text-[#28de91] font-bold text-body italic">
    ...
    <div className="max-w-7xl mx-auto">
      <div className="grid lg:grid-cols-2 gap-24 items-start">
        <div className="space-y-12">
          <h2 className="text-h2">{content.trust.title}</h2>
        ...
        <div className="bg-white rounded-[3rem] p-12 relative overflow-hidden min-h-[500px] shadow-2xl border-2 border-[#28de91]">
          ...
          <div className="relative z-10 h-full flex flex-col">
            <div className="mb-8">
              <p className="text-body font-bold uppercase tracking-[0.2em] text-gray-400 mb-4">{language === 'en' ? 'Ready to Start?' : 'Prêt à commencer ?'}</p>
              <h3 className="text-h3 font-serif mb-4">{language === 'en' ? 'Start Investing Today' : 'Commencez à investir'}</h3>
              <p className="text-gray-600 leading-relaxed">
                {language === 'en' ? 'Join thousands of investors building wealth through Dubai real estate. Start from 4,000 AED.' : 'Rejoignez des milliers d\'investisseurs qui construisent leur patrimoine grâce à l\'immobilier de luxe à Dubai. Dès 4 000 AED.'}
              </p>
            </div>

            <ul className="space-y-4 mb-10 flex-1">
              {(language === 'en' ? ['2-minute signup', 'Start from 4,000 AED', 'Secure SPV ownership', '100% remote process'] : ['Inscription en 2 minutes', 'Dès 4 000 AED', 'Propriété légale sécurisée', '100% à distance']).map((item, i) => (
                <li key={i} className="flex items-center gap-4 text-body font-medium text-gray-700">
            ...
            <div className="space-y-4">
              <a href="https://investdubai.my.smartcrowd.ae/" target="_blank" rel="noopener noreferrer" className="w-full bg-[#28de91] text-black py-5 rounded-full font-bold uppercase tracking-widest text-body hover:bg-[#20c47f] transition-all shadow-lg shadow-[#28de91]/20 block text-center">
                {language === 'en' ? 'Get Started' : "S'inscrire"}
              </a>
              <a href="https://calendly.com/acheffi/invest-dubai" target="_blank" rel="noopener noreferrer" className="w-full py-4 rounded-full font-medium text-body text-gray-500 hover:text-gray-700 transition-colors flex items-center justify-center gap-2">
                <Phone size={16} />
  ...
  return (
    <section id="faq" className="py-32 px-6 bg-white">
      <div className="max-w-4xl mx-auto">
        <div className="mb-20">
          <h2 className="text-h2 mb-6">{content.faq.title}</h2>
  ...
  <section className="py-32 px-6 bg-[#f9f9f8]">
    <div className="max-w-7xl mx-auto bg-black rounded-[4rem] p-16 md:p-32 text-white text-center relative overflow-hidden">
      ...
      <div className="relative z-10 space-y-10 max-w-3xl mx-auto">
        <h2 className="text-h2">{content.nextStep.title}</h2>
        <p className="text-body-lg text-white/60">
          {content.nextStep.subtitle}
        </p>
        
        <div className="flex flex-col sm:flex-row gap-6 justify-center pt-8">
          <a href="https://investdubai.my.smartcrowd.ae/" target="_blank" rel="noopener noreferrer" className="bg-[#28de91] text-black px-12 py-6 rounded-full font-bold uppercase tracking-widest text-body hover:bg-[#20c47f] transition-all transform hover:scale-105 shadow-2xl shadow-[#28de91]/20">
            {language === 'en' ? 'Get Started' : "S'inscrire"}
          </a>
          <a href="https://calendly.com/acheffi/invest-dubai" target="_blank" rel="noopener noreferrer" className="bg-transparent border border-white/20 hover:bg-white/5 px-12 py-6 rounded-full font-bold uppercase tracking-widest text-body transition-colors flex items-center justify-center gap-3">
            <Phone size={16} className="text-[#28de91]" />
            {language === 'en' ? 'Book a Call with an Advisor' : 'Appeler un conseiller'}
          </a>
        </div>
        
        <p className="text-white/30 text-body font-bold uppercase tracking-[0.2em] pt-8">
...
const Footer = ({ content }: { content: LanguageContent }) => (
  <footer className="py-12 md:py-20 px-6 bg-white border-t border-gray-100">
    <div className="max-w-7xl mx-auto">
      <div className="grid md:grid-cols-4 gap-16 mb-20">
        <div className="col-span-2 space-y-8">
          <Logo />
          <p className="text-body-lg text-gray-500 max-w-sm">
            {content.footer.description}
          </p>
        </div>
        <div>
          <h4 className="font-bold text-body uppercase tracking-[0.2em] mb-8 text-black">{content.footer.explorationTitle}</h4>
          <ul className="space-y-4 text-body text-gray-500 font-medium">
        ...
        <div>
          <h4 className="font-bold text-body uppercase tracking-[0.2em] mb-8 text-black">{content.footer.legalTitle}</h4>
          <ul className="space-y-4 text-body text-gray-500 font-medium">
      ...
      <div className="pt-12 border-t border-gray-100 space-y-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 text-body text-gray-400 font-bold uppercase tracking-[0.2em]">
        ...
        <p className="text-body text-gray-400 leading-relaxed max-w-5xl">
