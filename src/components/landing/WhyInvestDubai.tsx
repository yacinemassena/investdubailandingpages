import { Wrench, Shield, Banknote, Eye, Scale, MapPin, Users, HardHat, Package } from 'lucide-react';

interface WhyInvestDubaiProps {
  label: string;
  title: string;
  subtitle: string;
  trustPoints: Array<{
...
const trustIcons = [Wrench, Shield, Banknote, Eye, Scale];

export const WhyInvestDubai = ({ label, title, subtitle, trustPoints, highlightCard, alignedIncentives }: WhyInvestDubaiProps) => {
  return (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-brand-dark rounded-3xl p-10 text-white">
              <div className="text-6xl font-bold mb-5">{highlightCard.stat}</div>
              <div className="text-2xl font-medium mb-8 leading-snug">{highlightCard.statLabel}</div>
              
              <div className="space-y-4 mb-8">
                {highlightCard.features.map((feature, i) => {
            ...
            <div className="bg-brand-surface rounded-3xl p-10 border border-brand-border">
              <h3 className="text-2xl font-bold text-brand-dark mb-5 leading-tight">{alignedIncentives.title}</h3>
              <p className="text-brand-muted leading-relaxed text-[15px]">
                {alignedIncentives.description}
              </p>
            </div>
          </div>
        </div>
