  ...
  return (
    <section className="py-12 md:py-20 bg-white">
      <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-12">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6 max-w-5xl mx-auto">
          {whyUs.pillars.map((pillar, i) => {
            ...
            return (
              <div 
                ...
                className={`bg-brand-surface rounded-2xl p-8 transition-all group text-center h-full flex flex-col ${
                  showShadow ? 'shadow-xl -translate-y-1' : 'hover:shadow-lg hover:-translate-y-1'
                }`}
              >
                <div className="w-14 h-14 rounded-2xl bg-brand-accent text-white flex items-center justify-center mb-6 shadow-lg group-hover:scale-110 transition-transform mx-auto">
                  <Icon className="w-7 h-7" />
                </div>
                <h3 className="text-h4 text-brand-dark mb-3">{pillar.title}</h3>
                <p className="text-brand-muted leading-relaxed max-w-sm mx-auto whitespace-pre-line">{pillar.description}</p>
              </div>
            );
          })}
        </div>
          ...
          {whyUs.extras.map((extra, i) => {
            ...
            return (
              <div key={i} className="flex items-center gap-3">
                ...
                <span className="text-body text-brand-dark">{extra}</span>
              </div>
            );
          })}
        </div>
